...
public class RecipeItemColour implements IRecipe {
	...
	/**
	 * Use if you want to see if the item stack is a dye
	 * {@link #getCraftingResult(InventoryCrafting)}
	 */
	 ...
	 * {@link RecipeHandler#registerCraftingRecipes()}
	 */
	...
	public boolean matches(InventoryCrafting inv, World worldIn) {
		...
		for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
			...
			if (!stackInSlot.isEmpty()) {
				...
				if (cont)
					continue;
				else if (stackInSlot.getItem() != targetItemStack.getItem()
						&& stackInSlot.getItem() != Item.getItemFromBlock(Blocks.AIR))
					randomItemDetected = true;
			}
		}
		//As long as there is a item there and the item is the item we want
		return !itemColour.isEmpty() && itemColour.getItem() == targetItemStack.getItem() && !randomItemDetected;
	}
	...
	public ItemStack getCraftingResult(InventoryCrafting inv) {
		...
		if (stack.getItem() != targetItemStack.getItem())
			...
			if (!hasDye)
				finalColour = 0xFFFFFF; // If there is no dye, reset the item colour to white
