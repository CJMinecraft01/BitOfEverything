...
public class TileEntityDoubleFurnace extends TileEntitySidedInventory implements ITickable {
	...
	public TileEntityDoubleFurnace() {
		super(new int[] { 4, 5 }, new int[] { 0, 1 }, new int[] { 2, 3 }, new int[] { 2, 3 }, new int[] { 2, 3 },
				new int[] { 2, 3 });
		this.burnWorker = new Worker(0, () -> {
			...
			if (!this.world.isRemote) {
				...
				this.world.setBlockState(this.pos,
						this.world.getBlockState(this.pos).withProperty(BlockDoubleFurnace.ACTIVATED, false), 3);
			}
		});
		this.cookWorker = new Worker(0, () -> {
			...
			if (!this.world.isRemote) {
				...
				ItemStack result = FurnaceRecipes.instance().getSmeltingResult(toSmelt).copy();
		...
		if (!this.world.isRemote) {
			...
			if (canSmelt()) {
				if (this.cookWorker.getMaxWork() == 0 && this.burnWorker.getMaxWork() != 0) {
					this.cookWorker.setMaxCooldown(200); // Change 200 to the duration in ticks
					this.cookWorker.setReversed(false);
				}
				if (TileEntityFurnace.isItemFuel(this.handler.getStackInSlot(2)) && this.burnWorker.getMaxWork() == 0) {
					this.world.setBlockState(this.pos,
							this.world.getBlockState(this.pos).withProperty(BlockDoubleFurnace.ACTIVATED, true), 3);
	...
	private boolean canSmelt() {
		...
		if (!this.handler.getStackInSlot(4).isItemEqual(result)) {
			...
			return count <= 64 && count <= this.handler.getStackInSlot(5).getMaxStackSize();
		}
		int count = this.handler.getStackInSlot(4).getCount() + result.getCount();
		return count <= 64 && count <= this.handler.getStackInSlot(4).getMaxStackSize();
	}
	...
	protected boolean isStackValid(int slot, ItemStack stack) {
		if (slot == 2 || slot == 3)
			return TileEntityFurnace.isItemFuel(stack)
					|| SlotFurnaceFuel.isBucket(stack) && stack.getItem() != Items.BUCKET;
		if (slot == 4 || slot == 5)
			return false;
