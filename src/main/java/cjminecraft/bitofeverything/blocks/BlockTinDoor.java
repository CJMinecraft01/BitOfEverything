...
public class BlockTinDoor extends BlockDoor {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (this.blockMaterial == Material.IRON) {
			...
			if (iblockstate.getBlock() != this) {
				...
				worldIn.playSound(
						playerIn, blockpos, ((Boolean) state.getValue(OPEN)).booleanValue()
								? BoeSoundHandler.TIN_FENCE_GATE_OPEN : BoeSoundHandler.TIN_FENCE_GATE_CLOSE,
		...
		if (iblockstate.getBlock() == this) {
			...
			if (iblockstate1.getBlock() == this && ((Boolean) iblockstate1.getValue(OPEN)).booleanValue() != open) {
				...
				worldIn.playSound((EntityPlayer) null, blockpos,
						open ? BoeSoundHandler.TIN_FENCE_GATE_OPEN : BoeSoundHandler.TIN_FENCE_GATE_CLOSE,
						SoundCategory.BLOCKS, 1.0F, 1.0F);
			}
		}
	}
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return state.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER ? Items.AIR
				: Item.getItemFromBlock(ModBlocks.tinDoor);
	}
