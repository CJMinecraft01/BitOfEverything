...
/**
 * A tin version of a fence gate. This class is taken from the
 * {@link BlockFence} class
 * 
 * @author CJMinecraft
 *
 */
public class BlockTinFenceGate extends BlockHorizontal {
	...
	public BlockTinFenceGate(String unlocalizedName) {
		super(Material.IRON, Material.IRON.getMaterialMapColor()); // Change for
																	// our block
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		worldIn.playSound(playerIn, pos, ((Boolean) state.getValue(OPEN)).booleanValue()
				? BoeSoundHandler.TIN_FENCE_GATE_OPEN : BoeSoundHandler.TIN_FENCE_GATE_CLOSE, SoundCategory.BLOCKS,
				1.0F, 1.0F);
		// worldIn.playEvent(playerIn,
		// ((Boolean)state.getValue(OPEN)).booleanValue() ? 1008 : 1014, pos,
		// 0);
		return true;
	}
		...
		if (!worldIn.isRemote) {
			...
			if (((Boolean) state.getValue(POWERED)).booleanValue() != flag) {
				...
				if (((Boolean) state.getValue(OPEN)).booleanValue() != flag) {
					// worldIn.playEvent((EntityPlayer)null, flag ? 1008 : 1014,
					// pos, 0);
					worldIn.playSound(null, pos,
							flag ? BoeSoundHandler.TIN_FENCE_GATE_OPEN : BoeSoundHandler.TIN_FENCE_GATE_CLOSE,
